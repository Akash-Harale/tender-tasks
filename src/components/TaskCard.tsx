...
import { MoreHorizontal, Calendar, Paperclip, MessageSquare } from 'lucide-react';
...
export default function TaskCard({ task, columnId, onDragStart }: TaskCardProps) {
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <span className={`text-xs px-2 py-1 rounded ${getStatusColor(task.status)}`}>
            <span className={`inline-block w-1.5 h-1.5 rounded-full mr-1 ${statusDotColor}`}></span>
            {task.status}
          </span>
